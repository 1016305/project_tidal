...
func sway_weapon(delta):
	...
	if Input.is_action_pressed("left"):
		position.x = lerp(position.x, start_pos.x - drift_max * delta, sway_speed_position/10)
	...
	rotation_degrees.x = lerp(rotation_degrees.x, start_rot.x - (mouse_movement.y * sway_ammount_rotation) * delta, sway_speed_rotation)
